...
SET checkout_source = 'storefront'
WHERE checkout_source IS NULL OR btrim(checkout_source) = '';

ALTER TABLE public.payment_attempts
    ALTER COLUMN checkout_source SET DEFAULT 'storefront',
    ALTER COLUMN checkout_source SET NOT NULL;

ALTER TABLE public.payment_attempts
    DROP CONSTRAINT IF EXISTS payment_attempts_checkout_source_check,
    ADD CONSTRAINT payment_attempts_checkout_source_check
        CHECK (checkout_source IN ('storefront', 'whatsapp'));
...
CREATE INDEX IF NOT EXISTS idx_payment_attempts_public_access
    ON public.payment_attempts (external_id, public_access_token_hash)
    WHERE public_access_token_hash IS NOT NULL;

DROP FUNCTION IF EXISTS public.finalize_payment_order(
    TEXT,
    TEXT,
    TEXT,
    TEXT,
...
BEGIN
    ...
    IF COALESCE(NULLIF(btrim(p_checkout_source), ''), 'storefront') NOT IN ('storefront', 'whatsapp') THEN
        RAISE EXCEPTION 'CHECKOUT_ITEMS_INVALID:Origem do checkout invalida.';
    END IF;
...
REVOKE ALL ON FUNCTION public.finalize_payment_order(
    TEXT,
    TEXT,
    TEXT,
    TEXT,
...
GRANT EXECUTE ON FUNCTION public.finalize_payment_order(
    TEXT,
    TEXT,
    TEXT,
    TEXT,
